...
def load_samples(file_path):
    data = loadmat(file_path)  # Load the .mat file using loadmat
    # Assuming the data is stored in a variable named 'samples' within the .mat file
    samples = data['x']  
    return samples
...
class DataLoader:
    ...
    def load_train_and_val(self, normalized=True):
        train_1 = load_data(data_file="train_class0.mat", label=0, normalize=normalized)
        train_2 = load_data(data_file="train_class1.mat", label=1, normalize=normalized)
    ...
    def load_test(self, normalized=True):
        test_1 = load_data(data_file="test_class0.mat", label=0, normalize=normalized)
        test_2 = load_data(data_file="test_class1.mat", label=1, normalize=normalized)
...
class MultiLayerPerceptron:
    ...
    def train(self, learning_rate=1e-2, n_epochs=1000, batch_size=-1):
        epoch = 0
...
def train_and_test(mlp_classifier):
    ...
    if training_data is None:
      print("Error: training_data is None. Check DataLoader implementation.")
    ...
    # train
    mlp_classifier.train(learning_rate=1e-2)
